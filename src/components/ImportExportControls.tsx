    ...
    <Card title="Drucken und exportieren" subtitle={printHint}>
      <div className="space-y-4">
        <div className="surface-muted rounded-2xl p-4">
          ...
          <div className="export-choice-list mt-3" role="group" aria-label="Dokumentausgabe wählen">
            {pdfOutputs.map((output) => (
              <button
                key={output.value}
                type="button"
                className={`export-choice ${selectedDocumentOutput === output.value ? "export-choice-active" : ""}`}
                aria-pressed={selectedDocumentOutput === output.value}
                onClick={() => setSelectedDocumentOutput(output.value)}
              >
                <ReportIcon />
                {output.label}
              </button>
          ...
          {selectedPdfOutput ? (
            <div className="export-format-row mt-4">
              <div>
                <p className="label">Format für „{selectedPdfOutput.label}“</p>
                <p className="themed-muted text-sm">Wähle eine Ausgabe, um die Datei zu erstellen.</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button type="button" className="button-primary gap-2" onClick={selectedPdfOutput.action}>
                  <ReportIcon />
                  Druck-PDF
                </button>
        ...
        <div className="surface-muted rounded-2xl p-4">
          ...
          <div className="export-choice-list mt-3" role="group" aria-label="Tabelleninhalt wählen">
            {tableOutputs.map((output) => (
              <button
                key={output.value}
                type="button"
                className={`export-choice ${selectedTableOutputValue === output.value ? "export-choice-active" : ""}`}
                aria-pressed={selectedTableOutputValue === output.value}
                onClick={() => setSelectedTableOutputValue(output.value)}
              >
                <DownloadIcon />
                {output.label}
              </button>
          ...
          {selectedTableOutput ? (
            <div className="export-format-row mt-4">
              <div>
                <p className="label">Format für „{selectedTableOutput.label}“</p>
                <p className="themed-muted text-sm">Wähle ein Format, um die Datei zu erstellen.</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {selectedTableOutput.formats.map((format) => (
                  <button
                    key={format.value}
                    type="button"
                    className={format.value === "ods" ? "button-primary gap-2" : "button-secondary gap-2"}
