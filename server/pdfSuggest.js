...
const isPdfSuggestRequest = (value) => {
...
const normalizeText = (text) => text.replace(/\f/g, "\n");
const sanitizeBlockText = (value) => normalizeText(value)
    .split("\n")
    .map(cleanupLine)
    .filter(Boolean)
    .filter((line) => !isScoreNoiseLine(line) && !isAdministrativeNoiseLine(line))
    .join("\n");
const isAdministrativeNoiseLine = (line) => /^(name|note|unterschrift|resultierende note|datum[, ]|gesamtpunktzahl|kriteriales bewertungsraster)$/i.test(line) ||
    /^q\d+\b/i.test(line) ||
    /^erwartungshorizont\b/i.test(line);
const isScoreNoiseLine = (line) => /^(bewertungs-?\s*einheiten|klausur-?\s*punkte|max\.?|erreichte|punkt-?zahl|lösungsqualität)$/i.test(line) ||
    /^\d+\s*be\b/i.test(line) ||
    /^\d+\s*[-–]\s*\d+\b/.test(line);
const isSummaryLine = (line) => /^(summe|gesamt(?:punktzahl|punkte| \(inhalt\)| \(darstellungsleistung\)| teil))/i.test(line);
const parseTrailingPoints = (line) => {
...
const stripTrailingPoints = (line) => cleanupLine(line.replace(/\(?\d{1,3}\)?\s*(?:p\.|be)?\s*$/i, "").replace(/\s+[–-]\s*$/, ""));
...
const buildTask = (title, description, expectation, maxPoints) => ({
    title: title.trim(),
...
const isSectionHeadingLine = (line) => /^(klausurteil\s+[a-z]\b|teil\s+[a-z]\b)/i.test(line);
const getSectionHeading = (line) => {
...
const isCategoryHeadingLine = (line) => /^(?:\d+\.\s*)?(inhaltliche leistung|sprachliche leistung|sprachliche leistung\s*\/\s*darstellungsleistung|darstellungsleistung(?:\/sprachliche leistung)?|kommunikative textgestaltung|ausdrucksvermögen\s*\/\s*verfügbarkeit sprachlicher mittel|sprachrichtigkeit|inhaltspunkte)$/i.test(line);
const normalizeCategoryTitle = (line) => cleanupLine(line.replace(/^\d+\.\s*/, "").replace(/\s*:\s*$/, ""));
const isTaskHeadingLine = (line) => /^(teilaufgabe\s+\d+[a-z]?\b|task\s+\d+\b)/i.test(line) ||
    /^(comment|letter to the editor|analysis|summary|introductory sentence|short answers)\b/i.test(line);
const parseTaskHeading = (line, fallbackIndex) => {
...
const summarizeLines = (lines, maxLines, maxLength) => lines
    .map(cleanupLine)
    .filter(Boolean)
    .slice(0, maxLines)
    .join(" ")
    .slice(0, maxLength);
const extractNumberedCriteriaTasks = (lines) => {
    const criterionIndices = lines.reduce((indices, line, index) => {
        if (/^\d+\s+/.test(line) && !isSummaryLine(line)) {
            indices.push(index);
        }
        return indices;
    }, []);
    if (criterionIndices.length === 0)
        return [];
    return criterionIndices.slice(0, MAX_TASKS_PER_SECTION).map((startIndex, index) => {
        ...
        return buildTask(`Kriterium ${headingMatch?.[1] ?? index + 1}`, rawDescription || `Aus PDF erkanntes Kriterium ${index + 1}.`, expectation || rawDescription, parseTrailingPoints(headingLine));
    });
};
const extractTasksFromBlock = (block) => {
    ...
    const taskIndices = lines.reduce((indices, line, index) => {
    ...
    if (taskIndices.length > 0) {
        return taskIndices.slice(0, MAX_TASKS_PER_SECTION).map((startIndex, index) => {
            ...
            return buildTask(headingMeta.title, headingMeta.description, summarizeLines(detailLines, 10, 700) || summarizeLines(blockLines, 4, 280), parseTrailingPoints(summaryLine ?? "") ?? headingMeta.maxPoints);
        });
    }
    const criteriaTasks = extractNumberedCriteriaTasks(lines);
    if (criteriaTasks.length > 0)
        return criteriaTasks;
...
const extractSections = (text) => {
    const lines = normalizeText(text)
        ...
        .map(cleanupLine)
        .filter(Boolean);
    const sectionIndices = lines.reduce((indices, line, index) => {
...
const adaptSuggestionToRequest = (suggestion, request) => {
...
export const handlePdfSuggestRequest = async (payload) => {
    ...
    if (findings.length > 0 && !payload.riskAcknowledged) {
        return createError(422, "sensitive_content_detected", "Die PDF enthält potenziell sensible Inhalte. Ohne ausdrückliche Bestätigung wird kein Strukturvorschlag erzeugt.", findings.map((finding) => finding.message));
    }
